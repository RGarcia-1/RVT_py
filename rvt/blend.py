...
def e4mstp(
    ...
    svf_temp = default.get_sky_view_factor(
        dem, resolution, compute_svf=True, compute_opns=True
    )
    ...
    svf_flat = default_2.get_sky_view_factor(
        dem, resolution, compute_svf=True, compute_opns=False
    )["svf"].squeeze()
    ...
    # 4) Coloured slope
    slope_arr = rvt.vis.slope_aspect(
        dem=dem, resolution_x=resolution, resolution_y=resolution
    )["slope"]
    # ------------------------------------------------------------------------------------------------------------------
