...
dem_path = Path(__file__).parent / "test_data/TM1_564_146.tif"
...
def test_rvt_slope_tile_by_tile() -> None:
    out_slope_float_path = Path(default_values.get_slope_path(dem_path, bit8=False))
    out_slope_8bit_path = Path(default_values.get_slope_path(dem_path, bit8=True))
    rvt.default.save_rvt_visualization_tile_by_tile(
        rvt_visualization=rvt.default.RVTVisualization.SLOPE,
    ...
    slope_8bit_arr = default_values.float_to_8bit(
        float_arr=slope_arr, visualization=rvt.default.RVTVisualization.SLOPE
    )

    assert np.allclose(slope_float_tile_by_tile_arr, slope_arr, equal_nan=True)
    assert np.allclose(slope_8bit_tile_by_tile_arr, slope_8bit_arr, equal_nan=True)


def test_rvt_hillshade_tile_by_tile() -> None:
    out_hillshade_float_path = Path(
        default_values.get_hillshade_path(dem_path, bit8=False)
    )
    out_hillshade_8bit_path = Path(
        default_values.get_hillshade_path(dem_path, bit8=True)
    )
    rvt.default.save_rvt_visualization_tile_by_tile(
        rvt_visualization=rvt.default.RVTVisualization.HILLSHADE,
        rvt_default=default_values,
        dem_path=dem_path,
        output_dir_path=dem_path.parent,
        save_float=True,
    ...
    hillshade_8bit_arr = default_values.float_to_8bit(
        float_arr=hillshade_arr, visualization=rvt.default.RVTVisualization.HILLSHADE
    )
    assert np.allclose(hillshade_float_tile_by_tile_arr, hillshade_arr, equal_nan=True)
    assert np.allclose(
...
def test_rvt_svf_tile_by_tile() -> None:
    out_svf_float_path = Path(default_values.get_svf_path(dem_path, bit8=False))
    out_svf_8bit_path = Path(default_values.get_svf_path(dem_path, bit8=True))
    rvt.default.save_rvt_visualization_tile_by_tile(
        rvt_visualization=rvt.default.RVTVisualization.SKY_VIEW_FACTOR,
        rvt_default=default_values,
        dem_path=dem_path,
        output_dir_path=dem_path.parent,
        save_float=True,
    ...
    svf_8bit_arr = default_values.float_to_8bit(
        float_arr=svf_arr, visualization=rvt.default.RVTVisualization.SKY_VIEW_FACTOR
    )
    assert np.allclose(svf_float_tile_by_tile_arr, svf_arr, equal_nan=True)
    assert np.allclose(svf_8bit_tile_by_tile_arr, svf_8bit_arr, equal_nan=True)


def test_rvt_multi_hillshade_tile_by_tile() -> None:
    out_mhs_float_path = Path(
        default_values.get_multi_hillshade_path(dem_path, bit8=False)
    )
    out_mhs_8bit_path = Path(
        default_values.get_multi_hillshade_path(dem_path, bit8=True)
    )
    ...
    mhs_8bit_arr = default_values.float_to_8bit(
        ...
        visualization=rvt.default.RVTVisualization.MULTI_HILLSHADE,
        x_res=dem_arr_dict["resolution"][0],
        y_res=dem_arr_dict["resolution"][1],
        no_data=dem_arr_dict["no_data"],
    )

    assert np.allclose(mhs_float_tile_by_tile_arr, mhs_arr, equal_nan=True)
    assert np.allclose(mhs_8bit_tile_by_tile_arr, mhs_8bit_arr, equal_nan=True)
